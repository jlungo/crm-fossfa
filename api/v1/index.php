...
$container['vtiger'] = function() use ($adb, $container) {
    $current_user = CRMEntity::getInstance('Users');
    $current_user->retrieveCurrentUserInfoFromFile($container['jwt']->data->userId);
    return new VtigerHelper($adb, $current_user);
};

// JWT Middleware for authentication
$app->add(new \Slim\Middleware\JwtAuthentication([
    "path" => ["/"], // Path to check for authentication
    "passthrough" => ["/authorize","/register"], // Skip authentication for this path
...
$app->post('/register', function ($request, $response, $args) use ($app, $container) {
    error_log(print_r(array('SSS-1'),true),3,'/var/lib/crm-tcraccc/error.log');
    $result =VtigerHelper::Register($_POST);
    if($result =='Duplicate User Exists'){
        $message  = ['success' => false, 'message' => $result];
    }else{
	   //print_R($result);  die("vf");
        $message  = ['success' => true, 'message' => 'You have been successfully Registered','userid'=>$result];
    } 
...
$app->post('/authorize', function ($request, $response, $args) use ($app, $container) {
    global $application_unique_key, $site_URL;
